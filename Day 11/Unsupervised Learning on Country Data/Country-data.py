...
clust = AgglomerativeClustering(n_clusters=None, distance_threshold=1.0)
clust.fit(country_scaled)
...
scores = []
for i in Ks:
    clust = AgglomerativeClustering(n_clusters=i, distance_threshold=None)
    clust.fit(country_scaled)
    scores.append(silhouette_score(country, clust.labels_))

# clust = AgglomerativeClustering(n_clusters=None, distance_threshold=1.0)
# #gives error because the data is unscaled.
# clust.fit(country) 
# print(silhouette_score(country, clust.labels_))
